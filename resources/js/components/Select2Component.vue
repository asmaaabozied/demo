...
export default {
  ...
  data() {
    return {
      items: [],
      selected: this.intValue(),
      selected_values: this.arrayValue(),
    }
  },
  mounted() {
    this.selected = this.intValue();
    this.selected_values = this.arrayValue();
    axios.get(this.remoteUrl, {
      params: {
        selected_id: this.intValue() || this.arrayValue()
      }
    })
        .then(response => {
  ...
  methods: {
    ...
    arrayValue() {
      ...
      if (typeof this.value == 'string') {
        if (/^[\[|\{](\s|.*|\w)*[\]|\}]$/.test(this.value)) {
          value = JSON.parse(this.value);
        }
      } else if (this.value && ['array', 'object'].includes(typeof this.value)) {
        value = this.value;
      }
